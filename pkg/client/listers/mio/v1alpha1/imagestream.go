...
type ImageStreamLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.ImageStream, err error)
...
// List lists all ImageStreams in the indexer.
func (s *imageStreamLister) List(selector labels.Selector) (ret []*v1alpha1.ImageStream, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.ImageStream))
	})
	return ret, err
}
...
type ImageStreamNamespaceLister interface {
	...
	Get(name string) (*v1alpha1.ImageStream, error)
...
// List lists all ImageStreams in the indexer for a given namespace.
func (s imageStreamNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.ImageStream, err error) {
...
// Get retrieves the ImageStream from the indexer for a given namespace and name.
func (s imageStreamNamespaceLister) Get(name string) (*v1alpha1.ImageStream, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("imagestream"), name)
	}
	return obj.(*v1alpha1.ImageStream), nil
}
